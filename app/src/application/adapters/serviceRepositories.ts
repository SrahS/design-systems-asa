...
import type { FirebaseInfrastructure } from "@/infrastructure/firebase";
import type { AuthTokenStorage } from "@/infrastructure/storage";
...
export type ServiceRepositoriesDependencies = FirebaseInfrastructure & {
...
export const createServiceRepositories = ({
  ...
  transactionsService,
  usersService,
}: ServiceRepositoriesDependencies): ServiceRepositories => {
  ...
  const attachmentRepository: AttachmentRepository = {
    list: async (userId) => {
      const attachments =
        await attachmentsTransactionService.getAttachments(userId);
    ...
    listByTransactionMap: async (userId) => {
      const legacyMap =
        await attachmentsTransactionService.getAttachmentsByTransactionMap(
          userId
        );
  ...
  const sessionStorage: SessionStorage = {
    saveSession: async (session) => {
      await authTokenStorage.persistAuthTokenResponse(toTokenResponse(session));
    },
    getSession: async () => {
      const tokenResponse =
        await authTokenStorage.getPersistedAuthTokenResponse();
      return tokenResponse ? toSession(tokenResponse) : null;
    },
    clearSession: authTokenStorage.clearPersistedAuthTokenResponse,
    saveUser: async (user) => {
      await authTokenStorage.persistAppUser(toLegacyUser(user));
    },
    getUser: async () => {
      const user = await authTokenStorage.getPersistedAppUser();
      return user ? toDomainUser(user) : null;
    },
    clearUser: authTokenStorage.clearPersistedAppUser,
    clearAll: authTokenStorage.clearAllPersistedAuth,
  };
